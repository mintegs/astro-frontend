...
export function validateInstagramData(
  data: InstagramData | null | undefined,
): string | null {
  if (!data || typeof data !== 'object') {
    return 'اطلاعات معتبری از فایل اینستاگرام یافت نشد.'
  }
  ...
  if (!hasAtLeastOneEntry) {
    return 'اطلاعات قابل استفاده‌ای از فایل اینستاگرام استخراج نشد.'
  }
...
export async function parseInstagramZip(file: File): Promise<InstagramData> {
  ...
  const hideStory = await parseLabelValueFile(
    loadedZip.file('connections/followers_and_following/hide_story_from.json'),
  ...
  const blocked = await parseLabelValueFile(
    loadedZip.file('connections/followers_and_following/blocked_profiles.json'),
